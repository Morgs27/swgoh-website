...
// Sort Users that haven't been updated recently
while($data = $result->fetch_assoc()){

    $last_updated = $data->data->last_updated;
    $time = strtotime($data['refresh_date']);
    $end_time = date('Y-m-d H:i:s', strtotime("+1 day", $time));
    $current_date = date('Y-m-d H:i:s');
    
    // If user hasn't been updated in the last day add to array
...
foreach($users as $user){
    ...
    foreach($units as $unit){
        ...
        if ($combat_type == 1){
            ...
            // Check There Are no records of the same character in the database to aviod duplicates
            $sql_check = "SELECT * FROM user_character_data WHERE Username = '$username' AND defId = '$defId'";
            $result_check = $conn->query($sql_check);
            if ( mysqli_num_rows($result_check) > 0 ){
                while( $data_check = $result_check->fetch_assoc()){
                    $delete_id = $data_check['user_character_id'];
                    $sql_delete = "DELETE FROM user_character_data WHERE user_character_id = '$delete_id'";
        ...
        if ($combat_type == 2){
            ...
            // Check There Are no records of the same character in the database to aviod duplicates
            $sql_check = "SELECT * FROM user_ship_data WHERE Username = '$username' AND defId = '$defId'";
            $result_check = $conn->query($sql_check);
            if ( mysqli_num_rows($result_check) > 0 ){
                while( $data_check = $result_check->fetch_assoc()){
                    $delete_id = $data_check['user_ship_id'];
                    $sql_delete = "DELETE FROM user_ship_data WHERE user_ship_id = '$delete_id'";
                    $result_delete = $conn->query($sql_delete);
                    echo $sql_delete;
                }
            }
